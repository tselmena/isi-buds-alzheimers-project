...
```{r}
raw_delta_48 <- map(all_tests_list, ~select(.x, BID, ends_with("_W48")))
comb_raw_delta_48 <- reduce(raw_delta_48, full_join, by = "BID")
outcome_df2 <- all_tests_list[[7]] |> select(BID, all_of("CDPOS_W240"))
final_combined_df_raw <- inner_join(comb_raw_delta_48, outcome_df2, by = "BID")
W48_df_raw <- full_join(final_combined_df_raw, C3_48, by = join_by(BID)) |> 
  na.omit() |> 
  select(BID, CDPOS_W240, ends_with("_W48"))

#write.csv(W48_df_raw, "W48_Raw_And_Delta_Data.csv")
```
...
```{r}
test_codes_to_run <- c("ADLPQPT", "ADLPQSP", "ADLTOTAL", 
                "CFIPT", "CFISP", "CFITOTAL", 
                "DIGIT", "FCSRT96", "LMIIA", "MMSE", "PACC")
