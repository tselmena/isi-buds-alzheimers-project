...
C3_raw <- read_csv("C:/Users/emili/OneDrive - University of Georgia/ISI-BUDS/project-week1/A4SecondaryAnalysis/A4SecondaryAnalysis/A4_ClinicalData/External Data/cogstate_battery.csv")
```
...
V1OUTCOME <- ADQS_raw  |> 
  filter(VISITCD == "001") |>
  select(SUBSTUDY, BID, QSTESTCD, QSSTRESN) |>
...
SUBJINFO_LEARN <- SUBJINFO_raw |>
  left_join(V6OUTCOME, by = "BID") |>
  left_join(V1OUTCOME |> 
      select(SUBSTUDY, BID, CDRSB, CFITOTAL, CFISP, CFIPT, ADLPQPT, ADLPQSP), 
    by = "BID") |>
  filter(SUBSTUDY == "LEARN") |> 
  mutate(
...
CDR_ind <- CDR_raw |> 
  select(c("SUBSTUDY", "BID", "CDGLOBAL","CDADTC_DAYS_T0")) |> 
  filter(BID %in% learn_bids) |>
...
C3_raw %>%
  filter(SUBSTUDY == "LEARN") |> 
  summarise(min_day = min(TDate_DAYS_T0, na.rm = TRUE),
            max_day = max(TDate_DAYS_T0, na.rm = TRUE))
...
```{r}
c3_clean <- C3_raw |>
  select(SUBSTUDY, BID, TDate_DAYS_T0, C3Comp) |>
  na.omit() |>
  distinct(BID, TDate_DAYS_T0, .keep_all = TRUE) |> 
  filter(BID %in% learn_bids) |>
  mutate(WEEK = TDate_DAYS_T0 / 7) |>
  select(SUBSTUDY, BID, WEEK, C3_SCORE = C3Comp, TDate_DAYS_T0)
...
c3_with_change <- inner_join(c3_clean, c3_baseline, by = "BID") |>
...
```{r}
LEARN_raw_delta_48 <- map(all_tests_list, ~select(.x, BID, ends_with("_W48")))
LEARN_comb_raw_delta_48 <- reduce(LEARN_raw_delta_48, full_join, by = "BID")
LEARN_outcome_df2 <- all_tests_list[[7]] |> select(BID, all_of("CDPOS_W240"))
LEARN_final_combined_df_raw <- inner_join(LEARN_comb_raw_delta_48, LEARN_outcome_df2, by = "BID")
LEARN_W48_df_raw <- full_join(LEARN_final_combined_df_raw, C3_48, by = join_by(BID)) |> 
  na.omit() |> 
  select(BID, CDPOS_W240, ends_with("_W48"))

write.csv(LEARN_W48_df_raw, "LEARN_W48_Raw_And_Delta_Data1.csv")
```
