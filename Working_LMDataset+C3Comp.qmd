...
COGSTATE <- read_csv("data/COGSTATE_COMPUTERIZED.csv")
```
```{r}
#| message: false

PTAU217 <- read_csv("data/biomarker_pTau217.csv")
```
...
```{r}
# Corrected extraction based on actual COGSTATE structure

# Based on your output, AVISIT "001" = Visit 1 (baseline)
# and AVISIT "015" appears to correspond to Visit 6
# Let's verify this mapping first

cat("Verifying VISIT to AVISIT mapping for C3Comp:\n")
visit_map <- COGSTATE %>%
  filter(TESTCD == "C3Comp") %>%
  distinct(VISIT, AVISIT) %>%
  arrange(VISIT)
print(visit_map)

# Extract C3Comp from COGSTATE with correct AVISIT values
COGSTATE_data <- COGSTATE %>%
  filter(AVISIT %in% c("001", "015")) %>%  # Visit 1 and Visit 6
  filter(TESTCD == "C3Comp") %>%
  select(BID, AVISIT, VALUE) %>%
  mutate(
  ...
  pivot_wider(
    ...
    values_from = VALUE,
    names_prefix = "C3Comp_"
  )

cat("\nCOGSTATE_data structure:\n")
str(COGSTATE_data)
cat("\nNumber of subjects with C3Comp data:", nrow(COGSTATE_data), "\n")

# For the long format - map AVISIT to VISITCD
cogstate_for_join <- COGSTATE %>%
  filter(TESTCD == "C3Comp") %>%
  filter(AVISIT %in% c("001", "015")) %>%
  mutate(
    # Map AVISIT to VISITCD to match ADQS format
    VISITCD = case_when(
      AVISIT == "001" ~ "001",  # Baseline
      AVISIT == "015" ~ "006"   # Visit 6 (week 15 maps to visit 6)
    )
  ) %>%
  select(BID, VISITCD, VALUE) %>%
  rename(C3Comp = VALUE)

cat("\nCOGSTATE long format data:\n")
print(head(cogstate_for_join))

# Complete pipeline for combining datasets
# Extract cognitive/functional variables from ADQS
ADQS_cognitive <- ADQS_raw %>%
  filter(VISITCD %in% c("001", "006")) %>%
  filter(QSTESTCD %in% c("CFISP", "CFIPT", "CFITOTAL", "PACC", "MMSE", 
...
# Combine all datasets (wide format)
combined_dataset <- SUBJINFO %>%
  left_join(ADQS_cognitive, by = "BID") %>%
  left_join(COGSTATE_data, by = "BID") %>%
  left_join(PTAU_data, by = "BID") %>%
...
# Check C3Comp columns
c3_cols <- names(combined_dataset)[grepl("C3Comp", names(combined_dataset))]
cat("\nC3Comp columns in wide format:", paste(c3_cols, collapse = ", "), "\n")

if(length(c3_cols) > 0) {
  cat("\nC3Comp data summary (wide format):\n")
  print(summary(combined_dataset[c3_cols]))
}

cat("\nC3Comp summary in long format:\n")
print(summary(combined_long$C3Comp))
